...
class HardAwareMMD:
    ...
    def compute_loss(
        ...
        loss = checkpoint(HardAwareMMD._gaussian_kernel_mmd, u_samples, e_samples, kernel_mul, kernel_num, use_reentrant=False)
...
# ============================================================================
# Spatially Aligned MMD (NEW: Maintains spatial correspondence)
# ============================================================================


class SpatiallyAlignedMMD:
    """
    Spatially-Aligned MMD for Uncertainty Calibration.

    Key Innovation: For each spatial location (patch), compute MMD between
    the uncertainty and error pixel distributions WITHIN that patch.

    This provides:
    1. Spatial correspondence: patch[i] uncertainty is matched to patch[i] error
    2. True distribution matching: MMD is computed on pixel values within each patch

    Loss formulation:
        L = Σ_i w_i * MMD(U_pixels_in_patch_i, E_pixels_in_patch_i)
    where:
        - U_pixels_in_patch_i, E_pixels_in_patch_i are pixel values within patch i
        - w_i = softmax(error_i / τ) for error-weighted focus on hard regions

    References:
        - Gretton et al. (2012): "A Kernel Two-Sample Test"
        - Local distribution matching for spatial consistency
    """
    ...
    def compute_loss(
        ...
        use_kernel_distance: bool = True,  # Kept for API compatibility, ignored
        kernel_bandwidth: float = 0.1,
        focus_on_hard: bool = True,
        hard_weight_power: float = 1.0,
    ) -> tuple[torch.Tensor, dict]:
        """
        Compute TRUE local MMD loss with spatial correspondence.

        For each patch location i:
            1. Extract uncertainty pixels: U_i = {u_1, u_2, ..., u_D} where D = patch_size²
            2. Extract error pixels: E_i = {e_1, e_2, ..., e_D}
            3. Compute MMD(U_i, E_i) using RBF kernel
            4. Aggregate with error-weighted sum

        Args:
            uncertainty: [B, H, W] Predicted uncertainty map (0~1)
            error: [B, H, W] Actual error map (0~1)
            patch_size: Size of local regions (default: 16, gives 256 pixels per patch)
            temperature: Softmax temperature for error-weighting
            kernel_bandwidth: RBF kernel bandwidth for local MMD
            focus_on_hard: If True, weight patches by their error magnitude
            hard_weight_power: Power for error-based weighting

        Returns:
            loss: Scalar local MMD loss
        ...
        # 1. Extract corresponding patches (spatial alignment preserved)
        u_patches, n_patches = SpatiallyAlignedMMD._extract_patches(uncertainty, patch_size)
        e_patches, _ = SpatiallyAlignedMMD._extract_patches(error, patch_size)

        # CRITICAL: Detach error to prevent calibration loss from affecting backbone
        # This ensures:
        # 1. Only BNDL learns to predict uncertainty (not backbone)
        # 2. ~50% memory savings (no gradient graph for error path)
        # 3. Calibration loss doesn't conflict with segmentation loss
        e_patches = e_patches.detach()

        # u_patches, e_patches: [N, D] where D = patch_size², N = total patches
        # CRITICAL: Same index i corresponds to same spatial location!

        if n_patches < 2:
            return torch.tensor(0.0, device=device, dtype=dtype, requires_grad=True), {}

        # 2. Compute LOCAL MMD for each patch
        # For efficiency, we vectorize the computation across all patches
        local_mmd_values = SpatiallyAlignedMMD._compute_batch_local_mmd(u_patches, e_patches, kernel_bandwidth)  # [N]

        # 3. Error-weighted aggregation (focus on hard regions)
        e_mean = e_patches.mean(dim=1)  # [N] - mean error per patch

        if focus_on_hard:
            # Weight patches by error magnitude (curriculum: hard regions matter more)
            raw_weights = (e_mean + 0.01) ** hard_weight_power
            # Softmax for normalization
            weights = F.softmax(raw_weights / temperature, dim=0)  # [N]
        else:
            # Uniform weighting
            weights = torch.ones(n_patches, device=device, dtype=dtype) / n_patches

        # 4. Weighted loss
        loss = (local_mmd_values * weights).sum()

        # Clamp for stability
        loss = torch.clamp(loss, min=0.0, max=10.0)

        # 5. Metrics for monitoring
        u_mean = u_patches.mean(dim=1)
        metrics = {
            "local_mmd_mean": local_mmd_values.mean().item(),
            "local_mmd_max": local_mmd_values.max().item(),
            "error_mean": e_mean.mean().item(),
            "uncertainty_mean": u_mean.mean().item(),
    ...
    @staticmethod
    def _compute_batch_local_mmd(
        u_patches: torch.Tensor,
        e_patches: torch.Tensor,
        bandwidth: float,
    ) -> torch.Tensor:
        """
        Compute local MMD for each patch in a vectorized manner.

        For each patch i, computes:
            MMD²(U_i, E_i) = E[k(u,u')] + E[k(e,e')] - 2*E[k(u,e)]

        where u, u' ∈ U_i (uncertainty pixels in patch i)
              e, e' ∈ E_i (error pixels in patch i)
              k(x,y) = exp(-|x-y|² / 2σ²) is the RBF kernel

        Args:
            u_patches: [N, D] uncertainty patches (D = patch_size²)
            e_patches: [N, D] error patches
            bandwidth: RBF kernel bandwidth

        Returns:
            local_mmd: [N] MMD value for each patch
        """
        N, D = u_patches.shape
        device = u_patches.device
        dtype = u_patches.dtype

        # For numerical stability
        sigma_sq = bandwidth**2 + 1e-8

        #
        # MEMORY-EFFICIENT: Use sampling instead of full kernel matrices
        # Instead of [D, D] matrices, we sample m pixels and compute [m, m]
        # This reduces memory from O(D²) to O(m²) where m << D
        #
        # For D=256 and m=64: 64x memory reduction!
        max_samples = 64  # Number of samples per patch (vs full D=256)

        # Sample indices (same for all patches for consistency)
        if D > max_samples:
            # Random sampling - use same indices for all patches in this call
            sample_indices = torch.randperm(D, device=device)[:max_samples]
            u_sampled = u_patches[:, sample_indices]  # [N, m]
            e_sampled = e_patches[:, sample_indices]  # [N, m]
            m = max_samples
        else:
            u_sampled = u_patches
            e_sampled = e_patches
        ...
        # Now compute MMD on sampled points - much smaller [N, m, m] tensors
        # Process all patches at once since m is small

        # 1. Within-U kernel: E[k(u, u')]
        u_diff = u_sampled.unsqueeze(2) - u_sampled.unsqueeze(1)  # [N, m, m]
        u_dist_sq = u_diff**2
        k_uu = torch.exp(-u_dist_sq / (2 * sigma_sq))
        mean_k_uu = k_uu.mean(dim=(1, 2))  # [N]
        del u_diff, u_dist_sq, k_uu

        # 2. Within-E kernel: E[k(e, e')]
        e_diff = e_sampled.unsqueeze(2) - e_sampled.unsqueeze(1)  # [N, m, m]
        e_dist_sq = e_diff**2
        k_ee = torch.exp(-e_dist_sq / (2 * sigma_sq))
        mean_k_ee = k_ee.mean(dim=(1, 2))  # [N]
        del e_diff, e_dist_sq, k_ee

        # 3. Cross kernel: E[k(u, e)]
        ue_diff = u_sampled.unsqueeze(2) - e_sampled.unsqueeze(1)  # [N, m, m]
        ue_dist_sq = ue_diff**2
        k_ue = torch.exp(-ue_dist_sq / (2 * sigma_sq))
        mean_k_ue = k_ue.mean(dim=(1, 2))  # [N]
        del ue_diff, ue_dist_sq, k_ue

        # 4. MMD² = E[k(u,u')] + E[k(e,e')] - 2*E[k(u,e)]
        mmd_squared = mean_k_uu + mean_k_ee - 2 * mean_k_ue
        mmd_squared = torch.clamp(mmd_squared, min=0.0)

        local_mmd = torch.sqrt(mmd_squared + 1e-8)

        return local_mmd

    @staticmethod
    def _extract_patches(tensor: torch.Tensor, patch_size: int):
        """Helper to extract non-overlapping patches while preserving order."""
        B, H, W = tensor.shape
        ...
        # Extract patches: [B, H//P, P, W//P, P] -> [B, H//P, W//P, P, P]
        patches = tensor.unfold(1, patch_size, patch_size).unfold(2, patch_size, patch_size)
        patches = patches.contiguous().view(B, -1, patch_size * patch_size)

        # Flatten batch: [B*N_patches, Patch_Area]
        # ORDER IS PRESERVED: patch[i] corresponds to spatial location floor(i / Wp), i % Wp
        patches = patches.view(-1, patch_size * patch_size)
        return patches, patches.shape[0]
...
class DomainAwareSoftMMD:
    ...
    def compute_loss(
        ...
        if feature_map is None:
            raise ValueError(f"[{tag}] domain_aware_soft_mmd requires feature_map. Ensure backbone features are passed from forward_image().")

        # Safeguard 2: Spatial alignment
        if feature_map.shape[-2:] != (H, W):
            logging.warning(f"[{tag}] Feature map size {feature_map.shape[-2:]} != error size {(H, W)}. Auto-interpolating feature map.")
        ...
        if enable_monitoring:
            ...
            if error_norm.numel() > 1 and diversity_norm.numel() > 1:
                try:
                    correlation = torch.corrcoef(torch.stack([error_norm, diversity_norm]))[0, 1].item()

                    # Log for debugging
                    logging.info(f"[DomainAware] Error-Diversity correlation: {correlation:.3f} (Error range: [{e_min:.3f}, {e_max:.3f}], Diversity range: [{f_min:.3f}, {f_max:.3f}])")

                    # Expected: 0.2 < correlation < 0.6 (complementary info)
                    if correlation > 0.8:
                        logging.warning(f"High correlation ({correlation:.3f}) detected! Feature diversity may not provide additional information.")
                    elif correlation < 0.1:
                        logging.warning(f"Low correlation ({correlation:.3f}) detected! Check if using correct feature layer (should be vision_features[-1]).")
                except Exception as e:
                    logging.debug(f"Could not compute correlation: {e}")

        # (C) Combined difficulty (theory-driven, normalized)
        combined_difficulty = (1 - diversity_weight) * error_norm + diversity_weight * diversity_norm

        # Metrics for TensorBoard
        metrics = {}
        if error_norm.numel() > 1 and diversity_norm.numel() > 1:
            try:
                correlation = torch.corrcoef(torch.stack([error_norm.detach(), diversity_norm.detach()]))[0, 1].item()
        ...
        if u_patches.shape[0] > n_patches_to_sample:
            ...
            sampled_indices = torch.multinomial(weights, num_samples=n_patches_to_sample, replacement=False)
...
class CKAComputer:
    ...
    def compute_patch_based_cka(
        ...
        if n_total_patches <= 1:
            ...
            return CKAComputer._compute_cka_core(u_flat, e_flat, use_linear_kernel, use_minibatch, minibatch_size)
    ...
    def _compute_cka_core(
        ...
        if use_linear_kernel:
            if use_minibatch and N > minibatch_size:
                cka_similarity = CKAComputer.compute_linear_cka_minibatch(X_centered, Y_centered, minibatch_size)
            else:
                cka_similarity = CKAComputer.compute_linear_cka_direct(X_centered, Y_centered)
        else:
            # RBF kernel CKA
...
class GramComputer:
    ...
    def compute_gram_loss(
        ...
        """
        raise NotImplementedError("Gram matrix matching is not yet implemented. Use 'mmd' or 'cka' for now. To implement Gram matching, uncomment and complete the code below.")
    ...
    def compute_patch_based_gram(
        ...
        """
        raise NotImplementedError("Patch-based Gram matching is not yet implemented. Use 'mmd' or 'cka' for now.")
...
class DistributionMatcher:
    ...
    def __init__(
        ...
        Args:
            method: 'mmd' | 'cka' | 'gram' | 'mmd_hard_aware' | 'domain_aware_soft_mmd' | 'spatial_mmd'
            ...
            temperature: softmax temperature (domain_aware_soft_mmd/spatial_mmd)
        ...
        """
        valid_methods = ["mmd", "cka", "gram", "mmd_hard_aware", "domain_aware_soft_mmd", "spatial_mmd"]
    ...
    def compute_loss(
        ...
        return self._compute_loss_impl(uncertainty, error, use_patches, feature_map, tag)
    ...
    def _compute_loss_impl(
        ...
        elif self.method == "spatial_mmd":
            return self._compute_spatial_mmd_loss(uncertainty, error)
        else:
    ...
    def _compute_loss_with_checkpoint(
        ...
        return checkpoint(self._compute_loss_impl, uncertainty, error, use_patches, feature_map, tag, use_reentrant=False)
    ...
    def _compute_spatial_mmd_loss(
        self,
        uncertainty: torch.Tensor,
        error: torch.Tensor,
    ) -> tuple[torch.Tensor, dict]:
        """
        Compute Spatially-Aligned MMD loss.

        Unlike standard MMD which treats patches as unordered sets,
        this method maintains spatial correspondence between uncertainty and error.
        This is crucial for SAM where prompt location determines the error distribution.
        """
        return SpatiallyAlignedMMD.compute_loss(
            uncertainty=uncertainty,
            error=error,
            patch_size=self.patch_size,
            temperature=self.temperature,
            use_kernel_distance=True,
