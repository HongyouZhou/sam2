...
class BNDLLoss(nn.Module):
            ...
            if "multistep_bndl_outputs" in outs:
                ...
                for bndl_outputs in bndl_outputs_list:
                    if bndl_outputs is not None:
                        step_loss += self._compute_kl_loss(bndl_outputs)
                        valid_steps += 1
    ...
    def KL_GamWei(Gam_shape, Gam_scale, Wei_shape_res, Wei_scale):
        ...
        Wei_shape_res = torch.clamp(Wei_shape_res, min=1e-6, max=10.0)
        ...
        if torch.any(torch.isnan(KL)) or torch.any(torch.isinf(KL)):
            return torch.tensor(0.0, device=KL.device, requires_grad=True)
        
        return -torch.clamp(KL.view(-1).mean(), min=-1000, max=1000)
    ...
    def _compute_kl_loss(self, bndl_outputs):
        def kl_term(wei_lambda, inv_k, name=""):
            ...
            if torch.any(torch.isnan(wei_lambda)) or torch.any(torch.isnan(inv_k)):
                return torch.tensor(0.0, device=wei_lambda.device, requires_grad=True)
        ...
        KL_x = kl_term(bndl_outputs["wei_lambda"], bndl_outputs["inv_k"], "pixel")
        ...
        if (self.use_global_w_kl and 
            ...
            KL_w = kl_term(bndl_outputs["wei_lambda_w"], bndl_outputs["inv_k_w"], "prompt_hyper_in")
        ...
        return KL_x * pixel_kl_weight + KL_w * prompt_kl_weight
