...
class ProCoLoss(nn.Module):
    """
    Read ProCo auxiliary loss prepared by the model in outs_batch and expose
    ...
    def forward(self, outs_batch: list[dict], targets_batch: torch.Tensor | None = None):
        ...
        for outs in outs_batch:
            ...
            for b in reversed(bndl_list):
                if b is not None and ("proco_aux_loss" in b):
                    if device is None:
                        device = b["proco_aux_loss"].device
                    total = total + b["proco_aux_loss"]
                    count += 1
                    break
        if device is None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        core = (total / count) if count > 0 else torch.tensor(0.0, device=device, requires_grad=True)
        return {CORE_LOSS_KEY: core, "proco_scalar": core.detach()}
