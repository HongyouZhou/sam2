...
def inference_with_uctta(
    ...
    checkpoint_interval = 50  # Save every 50 videos
    ...
    for v_idx, vid in enumerate(video_names, 1):
        ...
        if collect_statistics and dataset_eval is not None and v_idx % checkpoint_interval == 0:
            import pickle
            import gc
            checkpoint_file = out_dir.parent / f".uctta_checkpoint_{dataset_name}_{v_idx:04d}.pkl"
            ...
            print(f"💾 UCTTA: Saved checkpoint ({v_idx}/{len(video_names)} videos) to {checkpoint_file.name}")
    ...
    if collect_statistics and checkpoint_files:
        import pickle
        print(f"\n🔄 UCTTA: Merging {len(checkpoint_files)} checkpoint files...")
        for checkpoint_file in checkpoint_files:
            try:
                ...
                print(f"Warning: Failed to merge UCTTA checkpoint {checkpoint_file}: {e}")
        print(f"✓ UCTTA: Merged all checkpoints")
